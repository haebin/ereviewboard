...
class PublishReviewRequestPage extends WizardPage {
	...
	public void createControl(Composite parent) {
		...
		bugsClosed.addModifyListener(new ModifyListener() {
			...
			public void modifyText(ModifyEvent e) {
				...
				reviewRequest.setBugsClosed(Collections
						.singletonList(bugsClosed.getText()));
		...
		_toUserComboAutoCompleteField = new AutoCompleteField(toUserText,
		...
		toUserText.addModifyListener(new ModifyListener() {
			...
			public void modifyText(ModifyEvent e) {
				...
				reviewRequest.setTargetPeople(Collections
						.singletonList(toUserText.getText()));
		...
		_toGroupComboAutoCompleteField = new AutoCompleteField(toGroupText,
				new TextContentAdapter(), new String[] {});
		...
		toGroupText.addModifyListener(new ModifyListener() {
			...
			public void modifyText(ModifyEvent e) {
				...
				reviewRequest.setTargetGroups(Collections
						.singletonList(toGroupText.getText()));
	...
	private Text newText(Composite layout) {
		...
		GridDataFactory.swtDefaults()
				.hint(PostReviewRequestWizard.TEXT_WIDTH, SWT.DEFAULT)
				.applyTo(toUserText);
		return toUserText;
	}
	...
	private Text newMultilineText(Composite layout) {
		...
		GridDataFactory.swtDefaults()
				.hint(PostReviewRequestWizard.TEXT_WIDTH, 60)
				.applyTo(description);
		return description;
	}
	...
	private boolean checkValid() {
		...
		if (reviewRequest.getSummary() == null
				|| reviewRequest.getSummary().length() == 0) {
			return false;
		}

		if (reviewRequest.getDescription() == null
				|| reviewRequest.getDescription().length() == 0) {
			return false;
		}

		if (reviewRequest.getTargetGroups().isEmpty()
				&& reviewRequest.getTargetPeople().isEmpty()) {
			return false;
		}
	...
	private String[] getUsernames() {
		...
		for (User user : _context.getReviewboardClient().getClientData()
				.getUsers())
	...
	private String[] getGroupNames() {
		...
		for (ReviewGroup group : _context.getReviewboardClient()
				.getClientData().getGroups())
